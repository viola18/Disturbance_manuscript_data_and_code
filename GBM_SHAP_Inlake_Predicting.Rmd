...
```{r read-data}
# Read
data  <- fread("no_missing_NLA_final.csv")
sem_out <- fread("SEM_predictions_resid_sim.csv")

inlake<-data

inlake$lake_shorelinedevfactor<-log(inlake$lake_shorelinedevfactor)
inlake$ws_area_ha<-log(inlake$ws_area_ha)
inlake$value.soil_clay_pct<-log((inlake$value.soil_clay_pct/100)/(1-(inlake$value.soil_clay_pct/100)))
inlake$value.soil_depthtobedrock_cm<-log(inlake$value.soil_depthtobedrock_cm+1)
inlake$value.soil_kffact<-log(inlake$value.soil_kffact+1)
inlake$value.soil_orgcarbon_gperkg<-log(inlake$value.soil_orgcarbon_gperkg+1)
inlake$nlcd_devsum_pct[inlake$nlcd_devsum_pct >=99] <- 99
inlake$nlcd_devsum_pct[inlake$nlcd_devsum_pct <=0.01] <- 0.01
inlake$nlcd_devsum_pct<-log((inlake$nlcd_devsum_pct/100)/(1-(inlake$nlcd_devsum_pct/100)))
inlake$ag_total[inlake$ag_total <=0.01] <- 0.01
inlake$ag_total<-log((inlake$ag_total/100)/(1-(inlake$ag_total/100)))
inlake$value.runoff_inperyr<-log(inlake$value.runoff_inperyr+1)
inlake$P_Ag_Buffered[inlake$P_Ag_Buffered >=99] <- 99
inlake$P_Ag_Buffered[inlake$P_Ag_Buffered <=0.01] <- 0.01
inlake$P_Ag_Buffered<-log((inlake$P_Ag_Buffered/100)/(1-(inlake$P_Ag_Buffered/100)))
inlake$DMR_Metals_TWPE_lb_eq_yr<-log(inlake$DMR_Metals_TWPE_lb_eq_yr+1)
inlake$DMR_Organic_Enrichment_Disch<-log(inlake$DMR_Organic_Enrichment_Disch+1)
inlake$Number_of_CSOs<-log(inlake$Number_of_CSOs+1)
inlake$SNFA_MEAN<-log(inlake$SNFA_MEAN+1)
inlake$PIMPV[inlake$PIMPV <=0.01] <- 0.01
inlake$PIMPV<-log((inlake$PIMPV/100)/(1-(inlake$PIMPV/100)))
inlake$Manure_P_kg_ha<-log(inlake$Manure_P_kg_ha+1)
inlake$HUC12_Pop<-log(inlake$HUC12_Pop+1)
inlake$lake_waterarea_ha<-log(inlake$lake_waterarea_ha)

inlake$mercury<-log(inlake$mercury)
inlake$atrazine<-log(inlake$atrazine+0.001)
inlake$microcystin<-log(inlake$microcystin+0.001)
inlake$ecoli<-log(inlake$ecoli+0.001)
inlake$chla<-log(inlake$chla)
inlake$tp<-log(inlake$tp)
inlake$tn<-log(inlake$tn)
inlake$secchi<-log(inlake$secchi)
inlake$DOC<-log(inlake$DOC)
inlake$conductivity<-log(inlake$conductivity)
...
train_set_atz$atrazine <- mk_labels(train_set_atz$atrazine, log(3+0.001))
...
test_set_atz$atrazine <- mk_labels(test_set_atz$atrazine, log(3+0.001))
...
X_train_mc <- as.matrix(inlake_train[, c(
  "value.soil_clay_pct", "SNFA_MEAN", "P_Ag_Buffered", "Manure_P_kg_ha",
  "value.runoff_inperyr", "ag_total", "value.soil_kffact", "value.soil_depthtobedrock_cm",
  "value.soil_orgcarbon_gperkg"
)])
Y_train_mc <- inlake_train[, c("microcystin")]
train_set_mc <- data.frame(microcystin = Y_train_mc, X_train_mc)
train_set_mc$microcystin <- mk_labels(train_set_mc$microcystin, log(1+0.001))
...
test_set_mc$microcystin <- mk_labels(test_set_mc$microcystin, log(1+0.001))
...
train_set_chl$chla <- mk_labels(train_set_chl$chla, log(30))
...
test_set_chl$chla <- mk_labels(test_set_chl$chla, log(30))
...
train_set_tp$tp <- mk_labels(train_set_tp$tp, log(100))
...
test_set_tp$tp <- mk_labels(test_set_tp$tp, log(100))
...
train_set_tn$tn <- mk_labels(train_set_tn$tn, log(3))
...
test_set_tn$tn <- mk_labels(test_set_tn$tn, log(3))
...
train_set_ecoli$ecoli <- mk_labels(train_set_ecoli$ecoli, log(100+0.001))
...
test_set_ecoli$ecoli <- mk_labels(test_set_ecoli$ecoli, log(100+0.001))
...
X_train_mercury <- as.matrix(inlake_train[, c(
  "value.soil_clay_pct", "value.soil_kffact", "value.soil_depthtobedrock_cm",
  "value.soil_orgcarbon_gperkg", "lake_shorelinedevfactor", "lake_waterarea_ha", "ws_area_ha",
  "PIMPV", "Number_of_CSOs", "HUC12_Pop", "nlcd_devsum_pct",
  "DMR_Organic_Enrichment_Disch", "DMR_Metals_TWPE_lb_eq_yr"
)])
...
train_set_mercury$mercury <- mk_labels(train_set_mercury$mercury, log(0.203))
...
test_set_mercury$mercury <- mk_labels(test_set_mercury$mercury, log(0.203))
...
train_set_DOC$DOC <- mk_labels(train_set_DOC$DOC, log(15.8))
...
test_set_DOC$DOC <- mk_labels(test_set_DOC$DOC, log(15.8))
...
ggsave("shap_grid.png", p_grid, width = 15, height = 25, dpi = 300)
```
